...
results = model.predict(source=main_img, conf=0.035, classes=[class_id])
...
# Calculate absolute bounding box coordinates relative to the original image dimensions
...
# Save the absolute bounding box details to a separate JSON file
absolute_bbox_path = os.path.join(output_dir, 'absolute_bbox_coordinates.json')
with open(absolute_bbox_path, 'w') as f:
    json.dump(absolute_bbox, f)
